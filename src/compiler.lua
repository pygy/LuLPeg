...
local function compile (pt, ccache)
    ...
    if pt.pkind ~= "grammar" then
        return compile(P{pt})
    end
    local decl, locals = {}, {}
    local acc = {"local ", external, " = ...\nlocal ", decl, [[
local main = function(sbj, si, caps, ci, state)
    local success, broken = true, false
    repeat]]}
    ...
    acc[2] = t_concat(external, ", ")
    local code = t_concat(acc, "\n")
end
